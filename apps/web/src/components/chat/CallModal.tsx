...
export default function CallModal() {
  ...
  const isRinging =
    (call.status === "calling" && call.peerOnline) ||
  ...
  useEffect(() => {
    if (call.status === "connected") {
      ...
      timerRef.current = setInterval(() => {
        setElapsed(
          Math.floor(
            (Date.now() - (connectedAtRef.current ?? Date.now())) / 1000,
          ),
        );
      }, 1000);
    } else {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 z-50 flex items-center justify-center ${isVideo ? "bg-black" : "bg-black/70 backdrop-blur-sm p-4"}`}
      >
        <div
          className={`bg-[#1e2433] overflow-hidden shadow-2xl ${isVideo ? "w-full h-full flex flex-col" : "w-full max-w-xs rounded-2xl border border-[#323a4d]"}`}
        >
          {/* Video area */}
          {isVideo ? (
              ...
              {call.status !== "connected" && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-[#1e2433]">
                  <div className="relative">
                    <Avatar
                      initials={call.peerName.slice(0, 2).toUpperCase()}
                      name={call.peerName}
                      size="xl"
                      src={call.peerAvatar}
                    />
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[#a3aed0] text-sm animate-pulse">
                      {statusLabel}
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-linear-to-b from-[#7269ef]/30 to-transparent pt-8 pb-6 flex flex-col items-center gap-3">
              ...
              <div className="text-center">
                ...
                <p
                  className={`text-sm mt-0.5 ${call.status === "connected" ? "text-green-400" : "text-[#a3aed0] animate-pulse"}`}
                >
                  {statusLabel}
                </p>
              </div>
          ...
          {/* Action buttons */}
          <div
            className={`flex items-center justify-center gap-6 shrink-0 ${isVideo ? "px-8 py-5 bg-black/80" : "px-8 py-6"}`}
          >
            {call.status === "incoming" ? (
              <>
                <div className="flex flex-col items-center gap-1.5">
                  <button
                    onClick={rejectCall}
                    className="w-14 h-14 bg-red-500 rounded-full flex items-center justify-center text-white hover:bg-red-600 transition-colors shadow-lg"
                  >
                    <RiPhoneLine size={24} className="rotate-135" />
                  </button>
                ...
                <div className="flex flex-col items-center gap-1.5">
                  <button
                    onClick={acceptCall}
                    className="w-14 h-14 bg-green-500 rounded-full flex items-center justify-center text-white hover:bg-green-600 transition-colors shadow-lg"
                  >
                    {isVideo ? (
                      <RiVideoLine size={22} />
                    ) : (
              ...
              <>
                ...
                {isVideo && (
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    <span className="text-[11px] text-[#6b7280]">
                      {call.isCameraOff ? "Show" : "Hide"}
                    </span>
                  </div>
                )}

                {/* Screen share (video only, connected only) */}
                {isVideo && call.status === "connected" && (
                  <div className="flex flex-col items-center gap-1.5">
                    <button
                      ...
                      className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors
                        ${call.isScreenSharing ? "bg-[#7269ef] text-white" : "bg-[#2a3042] text-[#a3aed0] hover:bg-[#323a4d]"}`}
                    >
                      <RiScreenshot2Line size={20} />
                    </button>
                    <span className="text-[11px] text-[#6b7280]">
                      {call.isScreenSharing ? "Stop" : "Share"}
                    </span>
                  </div>
                )}
