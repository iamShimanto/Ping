...
export default function UserInfoPanel({ contact, onClose }: UserInfoPanelProps) {
  return (
    <div className="w-72 bg-[#2a3042] border-l border-[#323a4d] flex flex-col h-full flex-shrink-0 overflow-y-auto">
      {/* Header */}
      <div className="p-4 flex items-center justify-between flex-shrink-0">
        <button onClick={onClose} className="text-[#6b7280] hover:text-[#a3aed0]">
          <RiCloseLine size={20} />
        </button>
      </div>
      ...
      <div className="px-5 py-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[10px] font-semibold text-[#6b7280] uppercase tracking-widest">Info :</p>
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-xs text-[#6b7280]">Name</p>
            <div className="flex items-center gap-2">
              <p className="text-xs text-[#a3aed0]">{contact.name}</p>
              <button className="text-xs text-[#7269ef] hover:underline">Edit</button>
            </div>
          </div>
          <div className="flex items-center justify-between">
            <p className="text-xs text-[#6b7280]">Email</p>
            <p className="text-xs text-[#a3aed0]">{contact.name.split(" ")[0]}@{contact.name.split(" ")[1]?.toLowerCase() || "mail"}.com</p>
          </div>
          <div className="flex items-center justify-between">
      ...
      <div className="px-5 py-4">
        ...
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-full border border-[#3d4554] flex items-center justify-center">
