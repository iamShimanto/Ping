      ...
      <div className="flex-1 overflow-y-auto px-3 sm:px-4 pb-20 sm:pb-4">
        {Object.entries(grouped).sort().map(([letter, group]) => (
          <div key={letter} className="mb-2">
            ...
            {group.map((c) => (
              <div key={c.id} className="flex items-center gap-3 px-2 py-2.5 sm:py-2 rounded-lg hover:bg-[#323a4d] active:bg-[#323a4d] group">
                <Avatar initials={c.initials} name={c.name} size="sm" />
                <span className="flex-1 text-sm text-[#a3aed0] font-medium truncate">{c.name}</span>
                <button className="opacity-0 group-hover:opacity-100 text-[#6b7280] hover:text-[#a3aed0] p-1">
                  <RiMoreLine size={16} />
                </button>
              </div>
            ))}
