...
import { useGetConversationsQuery, useAddNewFriendMutation } from "../../../api/conversation/conversationApi";
...
export default function ContactsPage() {
  ...
  const { data: searchResults = [], isFetching } = useSearchUsersQuery(search.trim(), {
    ...
    <div className="w-full sm:w-[300px] bg-[#2a3042] flex flex-col h-full border-r border-[#323a4d] shrink-0">
      ...
      <div className="flex-1 overflow-y-auto px-3 sm:px-4 pb-20 sm:pb-4">
        ...
        {isSearching && (
          <>
            {isFetching && (
              <p className="text-xs text-[#6b7280] px-1 py-2 animate-pulse">Searching…</p>
            )}
            ...
            {!isFetching && searchResults.length > 0 && (
              <div className="mb-3">
                ...
                {searchResults.map((u) => (
                  <div key={u._id} className="flex items-center gap-3 px-2 py-2.5 sm:py-2 rounded-lg hover:bg-[#323a4d] group">
                    <Avatar
          ...
          <>
            {friends.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-xs text-[#4b5563] mt-1">Search to find and message users</p>
              </div>
            ) : (
              Object.entries(grouped).sort().map(([letter, group]) => (
                <div key={letter} className="mb-2">
                  ...
                  {group.map((f) => (
                    <div key={f._id} className="flex items-center gap-3 px-2 py-2.5 sm:py-2 rounded-lg hover:bg-[#323a4d] group">
                      <Avatar
                      ...
                      />
                      <div className="flex-1 min-w-0">
                        <span className="text-sm text-[#a3aed0] font-medium truncate block">{f.fullName}</span>
                        <span className={`text-xs truncate block capitalize ${f.status === "online" ? "text-green-400" : "text-[#6b7280]"}`}>
                          {f.status}
                        </span>
                      </div>
                      <button
